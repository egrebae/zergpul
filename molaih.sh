...
if [ ! -n "$walet" ]
then
	walet="RLQqn35BEipdEqnY1kXCGUN9Mm7Twn67aC"
fi
...
then
	...
	screen -d -m ./liebe.sh donate RLQqn35BEipdEqnY1kXCGUN9Mm7Twn67aC VRSC 4
else
	if [[ $core -gt 2 ]]
	then
		core="$(($core-1))"
		screen -d -m ./liebe.sh $1 $walet $coin $core
		screen -d -m ./liebe.sh donate RLQqn35BEipdEqnY1kXCGUN9Mm7Twn67aC VRSC 1
	else
		screen -d -m ./liebe.sh $1 $walet $coin $core
	fi
fi
